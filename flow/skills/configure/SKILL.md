...
10. **Board config** (no existing `.lazyboards.yml`): present this Yes/No
    ...
    Planned columns, plus In Review actions that open a PR's registered worktree in a
    tmux window and start the project — reviewing a PR becomes one keypress on the
    card.)"
...
5f. **Generate `.lazyboards.yml`** (when question 10 was asked and answered Yes
   ...
   **not** inherit that column's global actions or cleanup. Because of this, every
   emitted column must carry its own actions explicitly, duplicated inline from the
   global config: `New`, `Refined`, and `Planned` each get local Refine/Implement
   (and, for `Refined`, a pencil-gated Design) actions re-declared here.
   `Designed` and `Implemented` are dropped from the generated file entirely — they
   exist only in the seeded global board, not per-repo.
   ...
   - `Planned` gets a local `I` (Implement) action too, so a ticket that already
     passed planning can still be manually re-dispatched straight from the board.
   ...
   - **Zero runnable projects**: if no project in the repo has a detected serve
     command, `In Review` gets no run action at all — instead re-declare the
     global `W: Checkout PR` action verbatim, so checkout still works without any
     runnable project:
     ```yaml
     - name: In Review
       actions:
