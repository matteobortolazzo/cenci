...
# `sandbox`-object merge script (#632, #1115). Modelled on
# detect-project.test.sh (a `failures=` counter, small assert_* helpers,
# mktemp fixture files, self-contained, auto-discovered by the flow gate's
# `*.test.sh` glob).
#
# Contract pinned here: merge-sandbox-config.sh takes an existing config
# (path or "-" for stdin), plus --dockerfile <true|false>,
# --dind <true|false> and --azure <true|false>, and prints the full merged
# config to stdout. Enabling/disabling any one sibling never touches the
# others' fields or unrelated keys; an emptied `sandbox` object is dropped
# entirely; both Claude's SKILL.md and Codex's codex.md must invoke this
# script (byte-equivalence between clients is guaranteed by construction —
# same script, same jq — not by prose parity).
#
# #1115 retires the `sandbox.baseVersion` config key: the script no longer
# accepts `--base-version` at all (it now hits the existing unknown-argument
# branch and exits 2), never writes `sandbox.baseVersion`, and deletes a
# legacy `sandbox.baseVersion` it encounters on the input config rather than
# preserving it (unlike an unrelated unknown sandbox sub-key, which
# `baseVersion` no longer is once configure retires the key it once owned).
set -uo pipefail
...
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile true --dind true --azure false
assert_eq "${CODE}" "0" "case1 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true,"dind":true}}' "case1 both siblings on"
rm -f "${CFG}"

# --- Case 2 (Q9/Q9b transition 2/4): dockerfile ON, dind OFF, from {} -------
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile true --dind false --azure false
assert_eq "${CODE}" "0" "case2 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true}}' "case2 dockerfile alone"
rm -f "${CFG}"

# --- Case 3 (Q9/Q9b transition 3/4): dockerfile OFF, dind ON, from {} -------
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile false --dind true --azure false
...
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile false --dind false --azure false
assert_eq "${CODE}" "0" "case4 exit"
assert_json_eq "${OUT}" '{}' "case4 both siblings off, sandbox omitted"
rm -f "${CFG}"

# --- Case 5: disabling dind removes only "dind", not enabled (and drops a --
# --- legacy baseVersion it finds along the way) -----------------------------
CFG="$(write_config '{"sandbox":{"enabled":true,"baseVersion":"22.04","dind":true}}')"
run_merge "${CFG}" --dockerfile true --dind false --azure false
assert_eq "${CODE}" "0" "case5 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true}}' "case5 dind-only-off preserves dockerfile sibling, drops legacy baseVersion"
rm -f "${CFG}"

# --- Case 6: disabling dockerfile removes enabled AND a legacy baseVersion, -
# --- not dind ----------------------------------------------------------------
CFG="$(write_config '{"sandbox":{"enabled":true,"baseVersion":"22.04","dind":true}}')"
run_merge "${CFG}" --dockerfile false --dind true --azure false
assert_eq "${CODE}" "0" "case6 exit"
assert_json_eq "${OUT}" '{"sandbox":{"dind":true}}' "case6 dockerfile-only-off preserves dind sibling, drops legacy baseVersion"
rm -f "${CFG}"

# --- Case 7: unknown top-level AND unknown sandbox sub-keys survive, but ----
# --- a legacy baseVersion (a key configure itself owned and is retiring) ---
# --- does not -----------------------------------------------------------------
...
assert_json_eq "${OUT}" '{"configVersion":"0.22.0","mcpServers":["angular"],"sandbox":{"enabled":true,"dind":true,"futureKey":"keep-me"}}' "case7 unknown top-level and unrelated sandbox sub-keys preserved, legacy baseVersion removed"
printf '%s' "${OUT}" | jq -e '.sandbox | has("futureKey") and (has("baseVersion") | not)' >/dev/null 2>&1 \
  || fail "case7 futureKey must survive while baseVersion must not (got: ${OUT})"
rm -f "${CFG}"
...
# --- Case 9 (#1115): --dockerfile true never writes sandbox.baseVersion ----
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile true --dind false --azure false
assert_eq "${CODE}" "0" "case9 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true}}' "case9 dockerfile-on writes only enabled, never baseVersion"
printf '%s' "${OUT}" | jq -e '.sandbox | has("baseVersion") | not' >/dev/null 2>&1 \
  || fail "case9 sandbox.baseVersion must never be written (got: ${OUT})"
rm -f "${CFG}"
...
if [[ "$(id -u)" -eq 0 ]]; then
  ...
  run_merge "${UNREADABLE_CFG}" --dockerfile true --dind true --azure false
...
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile true --dind false --azure true
assert_eq "${CODE}" "0" "case12 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true,"azure":true}}' "case12 azure on"
rm -f "${CFG}"
...
# answer or dind.
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile false --dind false --azure true
...
# --- (and drops a legacy baseVersion it finds along the way) ---------------
CFG="$(write_config '{"sandbox":{"enabled":true,"baseVersion":"22.04","dind":true,"azure":true}}')"
run_merge "${CFG}" --dockerfile true --dind true --azure false
assert_eq "${CODE}" "0" "case14 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true,"dind":true}}' "case14 azure-only-off preserves dockerfile and dind siblings, drops legacy baseVersion"
...
run_merge "${CFG}" --dockerfile true --dind false
...
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile true --dind false --azure yes
assert_eq "${CODE}" "2" "case17 non-boolean --azure must exit 2"
rm -f "${CFG}"

# --- Case 18 (#1115): a pre-existing legacy sandbox.baseVersion is removed -
# --- by a --dockerfile true merge, even with no other sibling change -------
CFG="$(write_config '{"sandbox":{"enabled":true,"baseVersion":"1.18.1"}}')"
run_merge "${CFG}" --dockerfile true --dind false --azure false
assert_eq "${CODE}" "0" "case18 exit"
assert_json_eq "${OUT}" '{"sandbox":{"enabled":true}}' "case18 legacy baseVersion removed by --dockerfile true"
printf '%s' "${OUT}" | jq -e '.sandbox | has("baseVersion") | not' >/dev/null 2>&1 \
  || fail "case18 legacy sandbox.baseVersion must be removed (got: ${OUT})"
rm -f "${CFG}"

# --- Case 19 (#1115): --base-version is an unknown argument, exits 2 -------
# The flag is retired entirely — it now falls through to the script's
# existing unknown-argument branch rather than being recognized and parsed.
CFG="$(write_config '{}')"
run_merge "${CFG}" --dockerfile true --dind false --azure false --base-version 1.2.3
assert_eq "${CODE}" "2" "case19 --base-version must exit 2 as an unknown argument"
grep -q -- "unknown argument: --base-version" <<<"${OUT}" \
  || fail "case19 must name --base-version in its unknown-argument error (got: ${OUT})"
rm -f "${CFG}"
