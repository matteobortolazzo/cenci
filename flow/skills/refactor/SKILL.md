...
For each approved finding, write the body to a temp file first (never use heredocs). Refactor has no ticket/PR identifier in scope, so `F<N>` alone would collide across concurrent runs — scope the filename with a `<run-id>` (a run-unique key generated once at skill start, e.g. a timestamp literal, which is inherently safe under the `shell-rules` scope-key format rule) in addition to the per-finding `F<N>` suffix:
...
---
*Generated by agentflow refactor analysis*' > /tmp/claude/agentflow-refactor-<run-id>-F<N>.md
BODY=$(cat /tmp/claude/agentflow-refactor-<run-id>-F<N>.md)
...
Review the findings above and run `/agentflow:refactor` again when ready to create tickets.
```
