...
Classify this diff into one of three review paths before launching any reviewer. Before evaluating any rule, delete/overwrite `/tmp/claude/cenci-<ticket-id-or-slug>-review-path.txt` (e.g. `rm -f`) so a stale value from an earlier run of this ticket can never be trusted if this classification is interrupted partway (e.g. context compaction mid-phase) — an absent file safely falls back to the "full trio" default documented in Phase 9. Evaluate the rules below **in order — first match wins**. Read `/tmp/claude/cenci-<ticket-id-or-slug>-files.txt` (changed file list) and `/tmp/claude/cenci-<ticket-id-or-slug>-stat.txt` (total changed lines) from the Shared Context step above.
...
- The diff content itself (added/removed lines in `/tmp/claude/cenci-<ticket-id-or-slug>-diff.patch`) contains one of the same keywords — this catches an innocuously-named file whose diff content adds something like `"apiSecret": "..."`.

0. **Toggle off**: if `.claude/config.json` sets `cenci.liteReviewEnabled` to `false` → path = **full**. Skip the remaining rules entirely.
...
3. **Small + safe**: if **all** of the following hold → path = **lite-small**:
   - Total changed lines (insertions + deletions, from `cenci-<ticket-id-or-slug>-stat.txt`) < 20.
...
4. **Otherwise** (ambiguous, over threshold, or any file off the allowlist) → path = **full**. Err toward full when uncertain. Ambiguity includes: `/tmp/claude/cenci-<ticket-id-or-slug>-files.txt` or `/tmp/claude/cenci-<ticket-id-or-slug>-stat.txt` is missing, empty, or the changed-line count cannot be confidently parsed as a number — resolve any of these to **full** before assuming 0 changed lines.

Write the chosen path string — `full`, `lite-docs`, or `lite-small` — to `/tmp/claude/cenci-<ticket-id-or-slug>-review-path.txt`.
