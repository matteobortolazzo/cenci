...
**If the routine itself cannot complete** — step 1's draft-validity check fails (e.g. a hard stop that occurred before a valid draft ever existed, such as a malformed draft assembly), or either pipeline call in steps 2/3 returns non-empty `errors[]`, or step 4's re-read verification fails — stop anyway, and report the residual state exactly as found rather than retrying indefinitely in this session. `watch/internal/dispatch/reconcile.go`'s `RecoveryResumeInterrupted` is the backstop for this residual case, and only once the tmux window is dead past its grace period — it is not the normal abort mechanism (per this ticket's Decisions). Name that backstop explicitly in the stop report, so a human reviewing the output knows the reconciler, not this session, will eventually recover it.

## Hard-Stop Inventory

Every hard stop after the `Working` claim across the three escalating sections below, with a stable ID, its trigger, and whether it funnels through `## Restore Awaiting-Input State` (all of them do except the sole justified exception, `HS-U0`). Each ID below is restated — not merely referenced — at exactly one call site in its owning section.
