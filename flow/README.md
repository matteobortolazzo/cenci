...
[cenci-watch](../watch/README.md) publishes live session state to tmux and optional
desktop surfaces.

## Start here

After the [one-command cenci install](../docs/getting-started.md), launch Claude Code
from the target repository and run:
...
<details>
<summary><strong>Generated maintainer inventories</strong> — skills, agents, dependencies, and docs</summary>
...
| `stack-go` | — | — | — | — |
| `subagent-safety` | — | — | — | — |
| `sync` | codex.md | codex-runtime, project-core, shell-rules, worktrees | — | — |
| `testing` | — | — | — | — |
| `ticket-ownership` | — | — | — | — |
...
`automerge:ok` is likewise a non-column marker: a per-ticket grant made by `/cenci:refine` at refinement time (never a repo-level default) that tells `/cenci:babysit` it may merge that ticket's PR without human review.
...
Not a usage control, but a related toggle documented here since it also changes planning session shape: top-level `"planning": { "autonomy": "lean" }` is a **safety/checkpoint toggle**, not a cost control. Default (unset, or any value other than the exact string `"lean"`): `"interactive"` — planning is unchanged from before this setting existed. `"lean"` lets the planner self-answer everything outside five named escalation classes (security-sensitive, destructive/irreversible, contradicts the refined ticket, genuine product ambiguity, scope blowup) and, when a plan comes back with no escalations, implicitly approves it and continues straight into Phase 2 in the same session (the Lean Approval Path — see the [board lifecycle](#board-lifecycle) section above). When lean planning *does* hit an escalation, it never falls back to an interactive prompt: it persists a draft plan (`status: awaiting-input`), posts the question as a ticket comment, swaps the board label to `Input Needed`, and stops — a human answers on the ticket, and `cenci dispatch` now relaunches it automatically once it detects that answer (a fresh manual `/cenci:implement` run also still picks the draft back up, for a repo not running the dispatch loop).
...
> `.mcp.json`. Re-run `/cenci:configure` and select Context7 to write it into the project's
> own `.mcp.json` (pinned to a specific version, with `CONTEXT7_API_KEY` read from the shell
> environment). The same run also heals any stale `mcp__plugin_cenci_context7__*` permission
> entries left over from the old plugin-scoped setup.
