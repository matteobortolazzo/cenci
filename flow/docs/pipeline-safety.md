...
- When refactoring or porting code from one path to another, if a prerequisite (e.g., schema loading, config parsing, data initialization) was previously loaded unconditionally or at a fixed point, verify that it is still loaded in *all* downstream code paths that depend on it — not just the path that contained it originally or the path where it was most recently called. A prerequisite moved from an unconditional-entry location to inside a conditional branch silently drops the prerequisite for other branches that still need it, turning the refactor into a hidden regression. Example: porting code that loads `get_app_state({ include_schema: true })` inside an "if design file selected" branch silently skips schema loading for the "from-scratch" path, even though both paths later call functions that require the schema. Trace every prerequisite (data fetches, config loads, initialization steps) that the original code performed before the refactored section, and verify each one is still reachable from *every* new code path, or explicitly document why a prerequisite can safely be skipped on a new path. (#957)
...
- When a pipeline ingests externally-sourced data (especially ticket body, comments, or other user-authored text in a public repo), any value that flows into command generation, script heredocs, or expression evaluation requires **both** (1) author/trust-gating via available API signals (e.g., GitHub's `authorAssociation: OWNER/MEMBER/COLLABORATOR`) to filter untrusted commenters, AND (2) format/charset validation against a strict whitelist (e.g., `^[A-Za-z0-9_-]{1,64}$` for IDs) before interpolation. Structural anchoring alone (e.g., "only bare-backticked tokens, never inline code") prevents accidental false positives but not adversarial crafting; neither gate is sufficient alone. Concrete instance: ticket #960 — an identifier sourced from public ticket comments needed both `authorAssociation` checks per comment and a charset whitelist to prevent injection into a downstream executable heredoc/expression. (The specific call site — context-gatherer.md's now-removed design-tool node-ID sourcing — no longer exists, but the general rule still applies to any comparably-sourced identifier.)

- When a security fix adds a validation gate, trust check, or protective pattern to one location that handles untrusted data, immediately audit every other location in the same codebase that consumes the same kind of data (including sibling options in the same decision tree, related checks in different phases, or mirrored procedures in other agents) to apply the same gate consistently. A partial fix that patches the first-discovered instance but misses a structurally identical sibling leaves the vulnerability class unresolved on one path. This audit must span the immediate change (the file being fixed) and every related file handling the same data type. Concrete instance: ticket #960 — a fix that added `authorAssociation` gating to one labeled-reference identifier source was incomplete until a follow-up pass applied the same gate to a sibling comment source *in the same file* AND propagated it to a related check elsewhere that read the same kind of ticket-carried reference for a different purpose. Only when every similar path is gated identically is the fix complete. (This concrete instance's call sites no longer exist — they were part of the now-removed design-tool integration — but the general audit rule still applies.)

- When a phase's sub-step reads a shared `$RUN_DIR/*` artifact file to gate its work (e.g., `docs-to-update.txt`, `files.txt`), distinguish between file absence ("no work to do") and read failure ("work identified but file unreadable"). Treat absence as a no-op skip; treat read failure as an error reported to the user (e.g., `docs: error (docs-to-update.txt unreadable)`). A file read that fails silently masks a requirement already identified and persisted by upstream phases, losing it without trace. This rule applies unconditionally — a later section in the same file may already demonstrate the correct pattern (e.g., a Maintenance Check sub-step), so never skip the guard by assuming adjacent code will catch the case.

- When a phase sub-step writes to a path specified by dynamic input (from config, implementer report, plan data, or other non-fixed source) rather than a hard-coded filename, add explicit path validation before any write: reject absolute paths, paths containing `..` segments, and paths outside the intended namespace (e.g., `docs/*.md` only, never `../../.github/workflows/`). Report and skip invalid entries rather than writing them. A data-driven write target offers no implicit protection against directory traversal; sibling sub-steps with fixed targets do not require this validation, so it is easy to overlook when modeling a new sub-step after them.
