...
| `worktree-isolation` | Script: `hooks/scripts/guard-main-worktree.sh` (wired via each adapter's own `hooks.json` under both a `Write\|Edit` matcher and a `Bash` matcher) | Writes must land only under `.worktrees/<id>-<desc>/`; the guard blocks `Write`/`Edit` calls targeting the main worktree, and (#795) also blocks Bash redirection (`>`, `>>`, `>|`, fd-prefixed/combined forms) and `tee`/`tee -a` write targets under the same allowlist. Both the Write\|Edit and Bash arms allow a canonicalized absolute target that resolves outside the repository's main worktree outright (#1072). Containment is measured against a shared `SCOPE_ROOT`, derived from the repository's main worktree (`git rev-parse --git-common-dir`'s parent, adopted only when that common dir is a genuine `.git` directory and its parent is a strict ancestor of the session's resolved root; otherwise falling back to that resolved root) rather than the per-session resolved root alone — so a write targeting the main worktree is still blocked even from a session running inside a feature worktree, closing a hole the pre-#1072 Bash-only out-of-root allowance shared with the (then Write\|Edit-only) main-worktree block. A narrow, best-effort self-protection denylist (`*/.claude/settings*.json`, `*/.claude.json`, `*/.claude/plugins/*`, `*/.ssh/*`) still blocks those specific out-of-root shapes on both arms, independent of repo containment; it is not a complete boundary (it does not model `mv`/`cp`/`sed -i`-style Bash write verbs or a symlinked `~/.claude`/`~/.ssh`). An extracted relative Bash write target is always rejected (#810): shell token composition and directory-changing commands can make the effective cwd differ from the hook cwd, a relative allowlist shape can resolve under a subdirectory instead of the repo root, and symlinks cannot be canonicalized safely without knowing that effective cwd. Callers use absolute feature-worktree, plan, design, or temp paths so the guard can canonicalize before deciding; unsupported zero-parse constructs retain the bounded raw-text backstop described next. Coverage boundary (#795/#810): the tokenizer is a precision layer, not a completeness proof — `cp`, `mv`, `dd of=`, `sed -i`, and `truncate` sit entirely outside this detection surface: these hooks do not inspect their write targets at all, so those targets are not protected by these hooks under --dangerously-skip-permissions. Each verb has a distinct target grammar with ambiguous target positions, and supporting them safely would materially expand parser complexity and false-positive risk without justifying the added maintenance cost. Where normal client permission rules are active they may add defense in depth, but they are never the sandbox boundary -- the container itself is. Additionally, heredoc bodies/arbitrary metacharacter gluing still degrade to the guard's empty-parse backstop (a raw-text scan for the repo root, with allowlisted-subtree mentions neutralized) and block — a false positive the agent can rephrase around, never a silent permit. Two constructs are now precisely modelled instead of falling to that backstop: an unquoted brace expansion in write-target position (e.g. `tee /tmp/x/{a,b}`) is detected directly by the tokenizer and fails the whole call closed with a dedicated exit code, distinct from the generic unmodelled-construct case; and `>` inside a well-formed, boundary-delimited `[[ ... ]]`/`(( ... ))` construct is recognized as comparison/arithmetic rather than a redirect — a real redirect immediately adjacent to such a construct, or a command/process substitution nested inside it in UNQUOTED text, is still correctly extracted, while the same kind of substitution found INSIDE a double-quoted string within the construct's interior is instead treated as an unsupported construct and fails closed with its own dedicated exit code. Backslash-newline removal is accounted for before recognizing nested-substitution and brace-range markers, matching Bash pre-tokenization behavior. Accepted residuals are named in `hooks/scripts/lib/bash-write-targets.sh`'s "Threat model and accepted residuals" header block. A recognized, strictly-shaped apply_patch envelope (#1036) is modelled directly: its declared Add/Update/Delete/Move targets are checked exactly like a Write/Edit target, including a relative-target exception (cwd-join-then-canonicalize, not the usual reject) that applies only under that strict shape, since no `cd`/`&&`/`;`/pipe/assignment/subshell can appear in it; a shell-composed apply_patch invocation (any of those operators, or trailing content after the envelope) is an accepted residual that falls back unchanged to the tokenizer above. Recognition covers every non-expanding heredoc delimiter spelling bash accepts (`<<'D'`, `<<"D"`, `<<\D`, their `<<-` variants, with any spacing around the operator) and the `<<-` tab-stripped body; an apply_patch invocation whose payload the parser cannot read at all (a here-string, an unclassifiable delimiter) fails closed rather than falling back (#1045). Claude: `phase-2-worktree.md`'s Create Worktree step (`git worktree add .worktrees/...`) plus `hooks/hooks.json` wiring. Codex: `codex.md`'s "create the worktree" step plus `codex/hooks.json` wiring (the same guard script, wired independently per adapter). | Claude, Codex |
...
| `sensitive-file-refusal` | Script: `hooks/scripts/check-sensitive-files.sh` (wired via each adapter's own `hooks.json` under both a `Write\|Edit` matcher and a `Bash` matcher) | Writes to env files, credentials/secrets/keys, and SSH/keystore files are blocked regardless of cenci configuration, whether via `Write`/`Edit` or (#795) via a Bash redirection/`tee` write target. Coverage boundary (#795/#810): same tokenizer as `worktree-isolation`'s Bash arm — `cp`, `mv`, `dd of=`, `sed -i`, and `truncate` sit entirely outside this detection surface: these hooks do not inspect their write targets at all, so those targets are not protected by these hooks under --dangerously-skip-permissions. Each verb has a distinct target grammar with ambiguous target positions, and supporting them safely would materially expand parser complexity and false-positive risk without justifying the added maintenance cost. Where normal client permission rules are active they may add defense in depth, but they are never the sandbox boundary -- the container itself is. Additionally, the same brace-expansion and comparison/arithmetic-context handling applies (each failing closed with its own dedicated exit code rather than the generic unmodelled-construct path). An unmodelled write-shaped construct (heredoc bodies, arbitrary metacharacter gluing) still degrades to this guard's own empty-parse backstop — a raw-text scan for its sensitive markers (`check_sensitive_raw`, kept in sync with the blocklist by the marker-sync contract test) — and blocks rather than silently allowing. Unlike `worktree-isolation`, this guard does NOT apply the relative-cwd-distrust policy: its decision axis is filename-marker matching, not path containment, so a stale-cwd relative target doesn't change whether the target matches a sensitive marker. A recognized, strictly-shaped apply_patch envelope (#1036) is modelled directly: its declared Add/Update/Delete/Move targets are checked against the same blocklist, inheriting this guard's existing (unchanged) `$(pwd)/<target>` relative-resolution join; a shell-composed apply_patch invocation is an accepted residual that falls back unchanged to the tokenizer above. Declared paths are treated as literal filenames on both guards (#1045): the heredoc body is never shell-expanded, so `$`, backticks, and parentheses in a declared path are ordinary filename characters rather than unresolved-expansion residue, and the `*** End of File` chunk marker is skipped rather than extracted as a phantom target. Both adapters wire the same script into their respective `hooks.json`. | Claude, Codex |
