...
|---|---|---|---|
| `baseline-gate` | Script: `hooks/scripts/run-gate.sh` | Claude: `skills/implement/phases/phase-2-worktree.md`'s Baseline Gate Check invokes `run-gate.sh` before Phase 3 and hard-stops on a red/unrunnable gate. Codex: **known gap** — `skills/implement/codex.md` never invokes `run-gate.sh`; tracked by #517's "Codex implement gate parity" child slice. `parity.test.sh` wires this as an explicit `xfail`, not a skip and not a silent pass. | Claude (real). Codex (gap, xfail). |
...
- **Codex `baseline-gate` / `gate-result-integrity`**: `codex.md` does not yet invoke
  `hooks/scripts/run-gate.sh` anywhere in its implement procedure, so there is no
  `GATE_STATUS=` output for it to interpret either. Both checks are currently,
  correctly, failing — `flow/tests/parity/parity.test.sh` wires them through its
  `xfail` primitive (reports `XFAIL`, does not count as a suite failure) rather
  than skipping or silently passing them. Fixing this gap — wiring `run-gate.sh`
  into the Codex implement procedure and giving it something to interpret — is
  tracked by **#517's "Codex implement gate parity" child slice**, not by this
  ticket. Once that slice lands, these two `xfail` markers in `parity.test.sh`
  must flip to plain assertions (an `XPASS` from the current markers is a hard
  failure by design, forcing that follow-up).
