...
- When a skill procedure needs to preserve existing ticket/issue body content while prepending or appending new content, capture the existing body via direct shell redirect (`gh ... --jq '.body' > file`) rather than having the model fetch-then-reproduce it through the `Write` tool — then compose the new body mechanically using `cat` to concatenate the files. Verify the full resulting content (via length comparison and/or full-body prefix check), not just a character-count prefix, especially when the write step is non-STOP/best-effort, where a verification gap silently passes a corrupted or truncated write that a prefix-only check would never catch. Four failure modes make this pattern sharper than it looks, and all four verify *clean* under the naive form of the checks above (#1059 review):
  - A shell redirect creates and truncates its target **before** the command runs, so a failed or partial fetch leaves a present-but-empty file that the `cat` then composes and the edit then posts as the resource's entire body. Guard the capture with `|| rm -f <file>`, exactly as a metadata fetch would be.
  - A post-edit length check compares the remote against *the file the capture produced*, so it cannot detect a bad capture — a truncated capture verifies clean while the body is destroyed. Gate the captured file against the live resource **before** composing anything from it, and treat gate failure as "the body was never touched" (the branch that is safe to skip).
  - A "retry the compose-and-edit sequence" instruction, applied after an edit that *succeeded* but failed verification, prepends the new content a second time — and the retry's own prefix and length checks then both pass. Any such retry must re-run the idempotency check first and recompose only when the content is confirmed absent.
  - Byte-exact length equality assumes a round-trip nothing establishes: `--jq '.body'` appends a newline the stored value did not have, and a web-UI-edited body can come back CRLF-delimited. Compare *normalized* lengths (fold CRLF, strip trailing newlines) computed identically on both sides, or the check fails on every good write — which then routes straight into the retry defect above.
