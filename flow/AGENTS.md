...
- Shell/jq/grep pitfalls (Bash tool CWD persistence across calls, jq `//` fallback semantics on empty strings, grep-based contract-test markers) are documented in `docs/shell-scripting-gotchas.md` — read before writing verification-critical commands, jq fallback chains, or grep-based contract tests.
