...
- Read `docs/shell-scripting-gotchas.md` before writing verification-critical shell commands, jq fallback chains, or grep-based contract tests. A fail-closed bugfix's own newly-introduced code is itself security-critical and needs the same review rigor as the bug it fixes (#812).
