# Synthetic adapter procedure (BROKEN — planning session writes a source file)

Same as fixtures/good-adapter/procedure.md except section 5 lets the planning
session write directly to a source file. The checker must fail this fixture
specifically for "planning session write immutability violated", while every
...
The planning session may write directly to a source file when it is convenient,
before any plan is saved and before any worktree exists.
