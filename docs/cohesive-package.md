...
Amendment (2026-07-26, #716): the installer installs and updates the lazyboards
**binary** only — it no longer seeds a default board config. Board config is per-repo
and committed (`.lazyboards.yml`, generated by `/cenci:configure`); the machine-global
`~/.config/lazyboards/config.yml` is no longer part of a cenci install. This
supersedes the seeding clause in the 2026-07-15 amendment above and leaves its
conclusion intact: distribution convenience, not layering.
