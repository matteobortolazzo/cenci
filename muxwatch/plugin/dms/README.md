...
```sh
ln -s "$PWD/plugin/dms" ~/.config/DankMaterialShell/plugins/muxwatch
```
...
Then open Settings (`dms ipc call settings toggle`) → **Plugins** → enable **MuxWatch** → **DankBar** → add the widget to a section.

## Behavior

- Polls every `pollIntervalMs` (default 2000 ms).
- Hides when muxwatch reports no sessions (or the daemon is down).
...
- Right-click is unused (open plugin settings via Settings → Plugins → MuxWatch).
