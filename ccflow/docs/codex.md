...
ccflow's implementation workflow is available to OpenAI Codex as a **documented
prose equivalent**, not a port. This is a deliberate design decision — see
[`docs/cohesive-package.md` §6.3](../../docs/cohesive-package.md#63-layer-3--ccflow-workflow)
...
- **Self-review** — the three ccflow reviewer conventions (security,
...
file. A single Codex agent performs every role — there are no ccflow skills or
subagents for it to call.

## What stays Claude-only, and why

ccflow is architecturally Claude-Code-only at every layer, and only the workflow
...
2. Merge [`templates/agentwatch-codex-config.json`](../templates/agentwatch-codex-config.json)
   ...
   **not** at `/ccflow:implement`, which is a Claude-only skill.
3. Launch:
