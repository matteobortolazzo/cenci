...
7b. **Pin subagents to 200K context**: "Do you want to pin ccflow's subagents to a 200K-context
    model? ccflow delegates reviews to subagents, and on 1M-context sessions that delegation can be
   ...
   - **Tiering caveat (state regardless of answer)**: the pin overrides every agent's `model:` frontmatter — ccflow's model tiering (opus planner/security-reviewer, haiku context-gatherer/structure-analyzer/lessons-collector) is flattened onto the pinned model while it is set. On a standard 200K session the 1M gate never fires, so answer No there to keep the tiering active.
