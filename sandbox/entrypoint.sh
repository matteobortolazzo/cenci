...
if [[ "$(id -u)" -eq 0 ]]; then
    ...
    if [[ -n "${CENCI_AGENT_CLI:-}" || -d /opt/cenci-agent ]]; then
        ...
        # This both fixes the path for downstream use AND acts as the
        # sentinel the dev-side block checks to avoid installing a second,
        # duplicate tee.
       ...
       && { [[ "${HOST_UID}" != "${CUR_UID}" ]] || [[ "${HOST_GID}" != "${CUR_GID}" ]]; }; then
        if [[ "${HOST_GID}" != "${CUR_GID}" ]]; then
            ...
            if [[ -n "${EXISTING_HOST_GROUP}" ]]; then
                usermod -g "${HOST_GID}" dev || { echo "remap: usermod primary group failed — the home volume may predate this cenci-sandbox version; try 'cenci sandbox prune --volumes'" >&2; exit 1; }
            else
                groupmod -g "${HOST_GID}" dev || { echo "remap: groupmod failed — the home volume may predate this cenci-sandbox version; try 'cenci sandbox prune --volumes'" >&2; exit 1; }
            fi
        fi
        if [[ "${HOST_UID}" != "${CUR_UID}" ]]; then
            usermod -u "${HOST_UID}" dev || { echo "remap: usermod failed — the home volume may predate this cenci-sandbox version; try 'cenci sandbox prune --volumes'" >&2; exit 1; }
        fi
        chown -R "${HOST_UID}:${HOST_GID}" /home/dev || { echo "remap: chown /home/dev failed — the home volume may predate this cenci-sandbox version; try 'cenci sandbox prune --volumes'" >&2; exit 1; }
...
# installed by the root block above (CENCI_BOOT_LOG survives the
# `sudo --preserve-env -u dev` re-exec into this script, since exec
# replaces the process image but inherited file descriptors persist) — so
# the tee below only installs as a FALLBACK, gated on CENCI_BOOT_LOG being
# unset, for the direct-as-dev entry point (e.g. a manual `docker run`
# without `--user root`, which skips the root block entirely — the image's
# default USER is dev). Installing it unconditionally here would pipe a
# second live tee through the first and double-write every line — do not
# "fix" that by dropping this guard. EXIT traps, unlike file descriptors, do
# NOT survive exec (trap state is shell-internal, discarded on process image
# replacement), so the trap below is always re-armed here regardless of
# which branch installed the tee. From here on, every subsequent
