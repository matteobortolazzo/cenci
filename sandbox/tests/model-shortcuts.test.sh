...
if ! grep -Eq "^exec -it -u dev .*claude-cenci-${REPO_SLUG} claude --dangerously-skip-permissions --model haiku -p test " "${CALLS_FILE}"; then
...
if ! grep -Eq "^exec -it -u dev .*codex-cenci-${REPO_SLUG} codex --dangerously-bypass-approvals-and-sandbox --model gpt-5\\.6-terra -p test " "${CALLS_FILE}"; then
...
if ! grep -Eq "^exec -it -u dev .*claude-cenci-${REPO_SLUG} claude --dangerously-skip-permissions --model sonnet -p ch " "${CALLS_FILE}"; then
...
EXEC_LINE="$(grep -E "^exec -it -u dev .*claude-cenci-${REPO_SLUG} claude " "${CALLS_FILE}" || true)"
if [[ -z "${EXEC_LINE}" ]]; then
