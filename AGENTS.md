...
- Never report a PR as CI-green from check `conclusion` values — a queued or running check has an *empty* conclusion, so a conclusion-only scan reads "still running" as "passed" (#900). Read `gh pr checks <pr> --repo <owner>/<repo> --json bucket,name,state`: green requires at least one check with every bucket `pass`; any `pending` check must be named as still running. See `flow/skills/shell-rules/SKILL.md`'s "Reading PR CI Status".
