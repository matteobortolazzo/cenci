// Package watch is the public, stdlib-only client for subscribing to a
// cenci daemon's live status stream.
//
...
// stable key external tools use to join cenci state onto their own data.
//
...
// cenci's internal packages, so it is safe to import from external tools.
// Versioning follows the existing watch/v* Go submodule tags.
package watch
