...
# real bootstrap log nor touches the developer's ~/.local/bin.
run_bootstrap() {
  local root="$1" calls="$2" logdir="$3" cenci_sandbox="$4"

  mkdir -p "$root/bin" "$root/.claude-plugin"
  cp "$PLUGIN_JSON" "$root/.claude-plugin/plugin.json"
...
STUB
  ...
  if [ -n "$cenci_sandbox" ]; then
    CLAUDE_PLUGIN_ROOT="$root" TMPDIR="$logdir" HOME="$home" CENCI_SANDBOX="$cenci_sandbox" \
      bash "$BOOTSTRAP"
  else
    (
      unset CENCI_SANDBOX
      export CLAUDE_PLUGIN_ROOT="$root" TMPDIR="$logdir" HOME="$home"
      bash "$BOOTSTRAP"
    )
  fi
}
