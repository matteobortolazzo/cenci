...
#      is handled by CenciWidget.qml's icon/color mapping
...
# bands) since CenciWidget.qml does not render per-agent budget
...
for c in $classes; do
    if grep -qE "case \"${c}\"" "$WIDGET"; then
        echo "ok   - CenciWidget.qml handles class '$c'"
    else
        echo "FAIL - class '$c' not handled in CenciWidget.qml"
        fail=1
    fi
done
...
if grep -qE 'cssAlt\s*=\s*j\["alt"\]' "$WIDGET"; then
    echo "ok   - CenciWidget.qml reads the alt field from poll JSON"
else
    echo "FAIL - CenciWidget.qml does not read the alt field from poll JSON"
    fail=1
fi

if grep -qE 'hasOutput\s*=\s*root\.cssAlt\s*!==\s*"none"' "$WIDGET"; then
    echo "ok   - CenciWidget.qml derives hasOutput from cssAlt (not cssClass)"
else
    echo "FAIL - CenciWidget.qml does not gate hasOutput on cssAlt"
    fail=1
fi

if grep -qE 'hasOutput\s*=\s*root\.cssClass\s*!==\s*"none"' "$WIDGET"; then
    echo "FAIL - CenciWidget.qml still gates hasOutput on cssClass (dispatch-only would be hidden)"
    fail=1
else
    echo "ok   - CenciWidget.qml does not gate hasOutput on cssClass"
fi
