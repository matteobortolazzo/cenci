...
- The `cenci` binary reachable by the widget — either on the Plasma session
...
```sh
~/.claude/plugins/marketplaces/cenci/watch/plugin/plasma/install.sh
# from a repo checkout, inside watch/: ./plugin/plasma/install.sh
```

It symlinks this plasmoid into
`~/.local/share/plasma/plasmoids/com.github.matteobortolazzo.cenci` and
restarts plasmashell so the change takes effect. It's idempotent — re-run after
any `cenci` update. You still add the widget to a panel once (below).
...
Then right-click the panel → **Add Widgets…** → search **Cenci** → add it.
Right-click the widget → **Configure Cenci…** to set the binary path.
...
ln -s "$PWD/plugin/plasma" \
  ~/.local/share/plasma/plasmoids/com.github.matteobortolazzo.cenci
...
- Hides from the panel when cenci reports `alt: "none"` (no sessions and
...
- The expanded view also shows a **Budget headroom** section (hidden when the
  ...
  that `cenci widget-json` appends to the tooltip is excluded from the session
...
| Key | Default | Notes |
|---|---|---|
| `pollIntervalMs` | `2000` | How often to run `cenci widget-json` (250–60000). |
| `cenciPath` | `cenci` | Path or command name for the binary. Use an **absolute path** if it is not on the Plasma session PATH. |

## Troubleshooting

- **Widget never appears / stays hidden**: run `cenci widget-json` in a terminal.
